...
exports._getCommandLineOptions = function(args) {
  var flags = {
      ...
      checkFlag, hasFlag, option, arg, argFilename;
  ...
  while ((args.length > 0) && (options.error === false)) {
    ...
    if (arg[0] !== '-') {
      ...
      if (options.file !== null) {
        options.error = 'Only one dotlit file is allowed.  Use --help for usage guidelines.';
      }
      options.file = arg;
    }
    // option with value?
    else if (['-e', '--extract', '-p', '--print', '-r', '--render'].indexOf(arg) !== -1) {
      // get the file
      argFilename = args.shift();
      if ((argFilename === undefined) || (argFilename.length === 0) || (argFilename[0] === '-')) {
        options.error = ['Missing filename for ', arg, '.  Use --help for usage guidelines.'].join('');
      }
      ...
      for (option in flags) {
        if (flags.hasOwnProperty(option)) {
          hasFlag = hasFlag || checkFlag(option, arg);
        }
      }
  ...
  if (options.file === null) {
    options.error = 'Missing dotlit filename.  Use --help for usage guidelines.';
  }
  
  // default to list if no other option given
  options.list = options.list || ((options.render === null) && !options.extractAll && (options.extractFiles.length === 0) && (options.printFiles.length === 0));
...
exports._processCommandLine = function() {
  ...
  if (options.help) {
    console.log([
        ...
        '  -r, --render          Renders the dotfile file as html',
        '  -v, --verbose         Shows messages while processing',
        '  -f, --force           Skips confirmation when extracting files outside the current directory',
  ...
  // if extracting all just grab all the files  
